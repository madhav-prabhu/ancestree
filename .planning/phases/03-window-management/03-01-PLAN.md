...
App should open with window positioned and sized correctly.
  ...
  <done>
Main process uses WindowStateManager for window creation. Window opens at persisted bounds (or centered 80% on first launch), tracks resize/move events, and cleans up on close.
  </done>
</task>

<task type="auto">
  <name>Task 3: Manual verification of persistence behavior</name>
  <files></files>
  <action>
Verify window state persistence works correctly:
...
Also verify edge case: Window should stay within minimum size constraints (cannot be resized smaller than 400x300).

Document the verification results in the summary.
  </action>
  <verify>
Run the app and manually verify persistence:
```bash
cd /home/madhav/projects/ancestree && npm run dev
```
Test sequence: Move window -> Close -> Reopen -> Verify position restored
  </verify>
  <done>
Window state persists across app restarts. First launch shows centered 80% window. Subsequent launches restore saved position/size. Minimum size enforced at 400x300.
  </done>
</task>

...
must_haves:
  truths:
    ...
    - "Menu events trigger corresponding hook methods in App.tsx"
  ...
  artifacts:
    ...
    - path: "src/App.tsx"
      provides: "Menu event integration with file operations"
      contains: "onMenuAction"
  ...
  key_links:
    - from: "src/App.tsx"
      ...
      via: "useFileOperations() hook call"
      pattern: "useFileOperations\\(\\)"
    - from: "src/App.tsx"
      to: "window.electronAPI.onMenuAction"
      via: "useEffect subscribing to menu events"
      pattern: "onMenuAction.*save|open|new"
...
<objective>
Create React hook for file operations, implement auto-save crash recovery system, and wire menu events to hook methods in App.tsx.

Purpose: Give the renderer a clean API for file operations (save, open, saveAs, new) with automatic dirty state tracking. Auto-save provides crash recovery so users don't lose work. Menu events from main process connect to hook methods for a complete workflow.

Output:
- `src/hooks/useFileOperations.ts` - React hook for file operations
- `src/App.tsx` - Menu event integration with hook
...
<task type="auto">
  <name>Task 4: Wire Menu Events to Hook in App.tsx</name>
  <files>src/App.tsx</files>
  <action>
Update `src/App.tsx` to integrate useFileOperations hook with menu events:
...
function App() {
  ...
  // Ref to hold current tree data for save operations
  const treeDataRef = useRef(/* your tree state */)

  // Update ref when tree data changes
  useEffect(() => {
    treeDataRef.current = /* your tree state */
    ...
    const unsubscribe = window.electronAPI!.onMenuAction(async (action) => {
      switch (action) {
        ...
        case 'open': {
          ...
          if (data) {
            // Load the data into your tree state
            // e.g., setTreeData(data as FamilyTreeData)
          }
          break
        }
  ...
  useEffect(() => {
    // When tree state changes, mark as dirty and update auto-save
    fileOps.markDirty()
    fileOps.updateAutoSave(/* tree state */)
  }, [/* tree state dependencies */, fileOps])
  ...
  useEffect(() => {
    ...
    const checkDraft = async () => {
      ...
      if (hasDraft && draft) {
        ...
        // If yes, load draft into tree state
        // Then clear draft
...
**Key wiring points:**
1. `useFileOperations()` hook provides all file operation methods
2. `useEffect` with `onMenuAction` subscribes to menu events and maps them to hook methods
3. Return unsubscribe function in cleanup to prevent memory leaks
4. Tree data changes trigger `markDirty()` and `updateAutoSave()`
5. On mount, check for crash recovery draft

**NOTE:** The exact tree state variable names depend on existing App.tsx structure. Adapt the code to match your state management (useState, useReducer, etc.).
  </action>
  <verify>
`npm run build` passes.
App.tsx imports useFileOperations.
App.tsx has useEffect that calls onMenuAction.
App.tsx calls markDirty and updateAutoSave when tree data changes.
  </verify>
  <done>
Menu events wired to file operations hook.
Complete circuit: Menu -> IPC -> onMenuAction -> hook methods.
Auto-save updates on data changes.
...
6. App.tsx imports and uses useFileOperations hook
7. Menu action -> hook method circuit complete:
   - File > Save triggers menu:save IPC
   - onMenuAction receives 'save'
   - fileOps.save() called with tree data
8. Tree data changes call markDirty() and updateAutoSave()
</verification>
...
- **Menu events reach hook methods via onMenuAction subscription**
- **Tree data changes trigger auto-save updates**

...
must_haves:
  truths:
    - "Renderer can invoke dialog:open and receive file data"
    - "Renderer can invoke dialog:save and write file to disk"
    - "Renderer can invoke dialog:saveAs and choose new location"
    - "Invalid JSON files return error instead of crashing"
  artifacts:
...
2. Add new `ALLOWED_RECEIVE_CHANNELS` array for menu events (prepare for Plan 02):
   - 'menu:new'
   - 'menu:open'
   - 'menu:save'
   - 'menu:saveAs'
...
3. Add `onMenuAction` method to electronAPI:
   ```typescript
   onMenuAction: (callback: (action: string) => void) => {
     // Listen for all menu channels, call callback with action name

...
2. Create a 1024x1024 PNG source icon using ImageMagick (REQUIRED dependency):
   - First verify ImageMagick is available: `which convert || (echo "ERROR: ImageMagick required. Install with: sudo apt install imagemagick" && exit 1)`
   - If ImageMagick is NOT available, STOP and inform the user to install it. Do not attempt alternative methods.
   - Create the placeholder icon: `convert -size 1024x1024 xc:'#f0fdf4' -fill '#166534' -gravity center -pointsize 400 -annotate 0 'A' assets/icon-source.png`
   - This creates a simple "A" for Ancestree as placeholder (user can replace with proper design)

3. Install electron-icon-builder as dev dependency:
   - `npm install -D electron-icon-builder`

4. Add icon generation script to package.json:
   ```json
   "icons": "electron-icon-builder --input=./assets/icon-source.png --output=./build"
   ```

5. Run the icon generation:
   - `npm run icons`
   - This creates build/icons/ directory with all PNG sizes
   - Creates build/icon.icns for macOS
   - Creates build/icon.ico for Windows

6. Copy the 512x512 PNG as build/icon.png for Linux fallback:
   - `cp build/icons/512x512.png build/icon.png`

IMPORTANT: ImageMagick is a hard requirement. If not installed, fail clearly with installation instructions rather than attempting workarounds.
  </action>
  <verify>
...
copyright: Copyright © 2026 Ancestree
...
- `grep "icon: build/icon.icns" electron-builder.yml` confirms macOS icon path matches Task 1 output
- `grep "icon: build/icon.ico" electron-builder.yml` confirms Windows icon path matches Task 1 output
- `cat package.json | grep '"pack"'` shows build scripts exist
  </verify>
  <done>electron-builder.yml exists with complete configuration for all platforms, icon paths correctly reference build/icon.icns and build/icon.ico from Task 1, package.json has pack:* scripts for building platform-specific installers</done>
</task>
...
- [ ] electron-builder.yml references correct icon paths (build/icon.icns, build/icon.ico)
...
3. electron-builder.yml icon paths match the generated icon files from Task 1
4. Build scripts in package.json allow platform-specific packaging
5. Running `npm run pack:linux` on Linux produces a runnable AppImage (best test without macOS/Windows)
</success_criteria>
